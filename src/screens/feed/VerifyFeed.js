...
import { getAllPosts, getProfilePosts, getUserPosts, getViewedPosts, postLive, removePost, updtaeViews } from '../../Apis/LaravelApis/postApi';
...
export default function VerifyFeed(props) {
    ...
    useEffect(() => {
        dispatch(activeFeedState('Verify'))
    ...
    const removePosts = (id) => {
        dispatch(removePost(id)).then((data) => {
            if (data?.status) {
                ...
                if ((newData?.length) == 0) {
                    RootNavigation.navigate('verifyPosts', {
                        from: "Verify",
                        videos: newData,
                        nextpage: nextPage,
                    })
    ...
    const livePost = (id) => {
        dispatch(postLive(`post/live/${id}`)).then((data) => {
            if (data?.status) {
                let tempData = [...posts];
                let newData = tempData.filter(function (el) {
                    return el.id != id;
